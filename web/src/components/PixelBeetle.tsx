...
/**
 * Desert-themed pixel dung beetle with layered animation:
 *  - Layer 1 (back): Desert scene — sand dunes, cactus, wind particles
 *  - Layer 2 (mid): Rolling dung ball (rotates)
 *  - Layer 3 (front): Brown beetle pushing the ball (bobs up/down)
 */
export default function PixelBeetle({
  ...
  return (
    <div className={`inline-block relative ${className}`} style={{ width: s, height: showScene ? s * 0.85 : s * 0.55 }}>
      ...
      >
        {/* === LAYER 1: Desert Background (only in scene mode) === */}
        {showScene && (
        ...
        {/* === LAYER 2: Dung Ball (rotates when rolling) === */}
        <g className={rolling ? "ball-roll" : ""} style={{ transformOrigin: "20px 17px" }}>
          {/* Ball body */}
        ...
        {/* === LAYER 3: Beetle (bobs when rolling) === */}
        <g className={rolling ? "beetle-push" : ""}>
          {/* Legs back */}
